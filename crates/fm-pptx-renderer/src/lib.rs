...
use ooxmlsdk_pdf::{PdfOptions, convert_pptx};
use thiserror::Error;
...
pub fn render_pptx_to_pdf(bytes: &[u8]) -> Result<Vec<u8>, PptxRenderError> {
    let pdf = convert_pptx(Cursor::new(bytes), PdfOptions::default())
        .map_err(|error| PptxRenderError::Conversion(error.to_string()))?;
